...
####################################################################
class AllFusionNetMultiplyDEChanelFeature(nn.Module):
    def __init__(self, args):
        super(AllFusionNetMultiplyDEChanelFeature, self).__init__()
        ...
        if self.use_residual:
            ...
            self.upsampleS5 = nn.Sequential(nn.Upsample(size=(100,1), mode='bilinear', align_corners=None), 
                                       nn.Conv2d(self.channels9, 8, kernel_size=(11,1), stride=(1,1), padding=(5,0), bias=False), nn.BatchNorm2d(8))
        ...
        if self.use_residual:
            self.Seq = nn.LSTM(8, self.hidden_size, self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=self.dropout) # as residual block
        else:
    ...
    def forward(self, x, y):
        ...
        if self.use_residual:
            ...
            x = x.view(-1, self.channels9, 100).transpose(1,2)
