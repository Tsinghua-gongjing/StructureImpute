...
####################################################################
class AllFusionNetMultiplyDE(nn.Module):
    def __init__(self, args):
        super(AllFusionNetMultiplyDE, self).__init__()
        ...
        if self.use_residual:
            ...
            self.S7G = nn.Sequential(nn.Conv2d(in_channels=self.channels8, out_channels=8, kernel_size=(15,1), stride=(5,1), padding=(0,0)), 
                                           nn.BatchNorm2d(8), 
                                           nn.ReLU(inplace=True),
                                           nn.MaxPool2d(kernel_size=(2,1), stride=(2,1)),
                                           nn.Conv2d(in_channels=8, out_channels=8, kernel_size=(3,1), stride=(2,1), padding=(1,0)), 
                                          )
            self.S8G = nn.Sequential(nn.Conv2d(in_channels=self.channels9, out_channels=32, kernel_size=(15,1), stride=(5,1), padding=(0,0)), 
                                           nn.BatchNorm2d(32), 
                                           nn.ReLU(inplace=True),
                                           nn.MaxPool2d(kernel_size=(2,1), stride=(2,1)),
    ...
    def forward(self, x, y):
        ...
        if self.use_residual:
            ...
            x = self.convS1(x)        
            y = self.convI1(y)
            # print("[Check shape] conv1",x.shape,y.shape)
            
            x = self.resblockS1(x)     
            
            # print("[Check shape] resblock1",x.shape,y.shape)
            ...
            x = self.resblockS7(x)
            x_S7G = self.S7G(x)
            # print("[Check shape] resblock1",x.shape,y.shape, x_S7G.shape)
            ...
            x_S8G = self.S8G(x)
            # print('[Check shape]', x.shape, y.shape, x_S8G.shape)
            ...
            y = y_conv2.view(-1, self.channels2, 100).transpose(1,2)
