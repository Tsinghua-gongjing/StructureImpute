...
####################################################################
class AllFusionNetMultiplyAlltime(nn.Module):
    def __init__(self, args):
        super(AllFusionNetMultiplyAlltime, self).__init__()
        ...
        self.fc = nn.Linear(self.hidden_size * self.direction * self.num_classes, self.num_classes)
    ...
    def forward(self, x, y):
        ...
        if self.lstm_all_time:
            out = self.fc(out3[:, :, :].view(-1,1,self.hidden_size * self.direction * self.num_classes))
        else:
