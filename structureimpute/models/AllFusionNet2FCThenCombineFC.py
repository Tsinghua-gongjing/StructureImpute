...
####################################################################
class AllFusionNet2FCThenCombineFC(nn.Module):
    def __init__(self, args):
        super(AllFusionNet2FCThenCombineFC, self).__init__()
    ...
    def forward(self, x, y):
        ...
        # Decode the hidden state of the last time step
            out1 = self.fc(out1[:, self.lstm_output_time, :]) #   => out = self.fc(out[:, :, :])
            out2 = self.fc2(out2[:, self.lstm_output_time, :])
            out = self.fc3(out1*out2)
