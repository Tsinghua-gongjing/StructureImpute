...
####################################################################
class AllFusionNetMultiplyRes2(nn.Module):
    def __init__(self, args):
        super(AllFusionNetMultiplyRes2, self).__init__()
        ...
        if self.use_residual:
            ...
            downsample11 = nn.Sequential(                   
                nn.Conv2d(self.channels2, self.channels3, kernel_size=1, stride=1, bias=False),                    
                nn.BatchNorm2d(self.channels3)
            )
            
            self.resblock11 = ResidualBlock(in_channels=self.channels2, 
                                          out_channels=self.channels3, 
                                          kernel_size=(11,1), padding=(5,0), 
                                          downsample=downsample11)
            ...
            downsample22 = nn.Sequential(                   
                nn.Conv2d(self.channels2, self.channels3, kernel_size=1, stride=1, bias=False),                    
                nn.BatchNorm2d(self.channels3)
            )
            
            self.resblock22 = ResidualBlock(in_channels=self.channels2, 
                                          out_channels=self.channels3, 
                                          kernel_size=(11,1), padding=(5,0), 
                                          downsample=downsample22)
        ...
        if self.use_residual:
            self.lstm = nn.LSTM(self.channels3, self.hidden_size, self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=self.dropout) # as residual block
        else:
        ...
        if self.use_residual:
            self.icSHAPE = nn.LSTM(self.channels3, self.hidden_size, self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=self.dropout)
        else:
    ...
    def forward(self, x, y):
        ...
        if self.use_residual:
            ...
            y_res = y * x_res
            x = self.resblock11(x)
            y = self.resblock22(y_res)
            x_res2 = x
            y_res2 = y * x_res2
            # print("[Check shape]",x.shape,y.shape)
            y = y_res2.view(-1, self.channels3, 100).transpose(1,2)
            x = x.view(-1, self.channels3, 100).transpose(1,2) # (N,100,4)
            # print("[Check shape]",x.shape,y.shape)
        ...
        if self.lstm_all_time:
            ...
            out = self.fc(out[:, self.lstm_output_time, :])
