...
####################################################################
class AllFusionNetMultiply(nn.Module):
    def __init__(self, args):
        super(AllFusionNetMultiply, self).__init__()
    ...
    def forward(self, x, y):
        ...
        h0 = torch.zeros(self.num_layers * self.direction, x.size(0), self.hidden_size).to(x.device)
        c0 = torch.zeros(self.num_layers * self.direction, x.size(0), self.hidden_size).to(x.device)
        
        h1 = torch.zeros(self.num_layers * self.direction, y.size(0), self.hidden_size).to(x.device)
        c1 = torch.zeros(self.num_layers * self.direction, y.size(0), self.hidden_size).to(x.device)
        ...
        # Decode the hidden state of the last time step
            out = self.fc(out3[:, self.lstm_output_time, :]) #   => out = self.fc(out[:, :, :])
        # print("[Check shape] out(after fc): {}".format(out.shape))
        if self.args.out_featuremap:
            print('fmap:','out1',out1.shape, 'out2',out2.shape, 'out3',out3.shape, 'out',out.shape)
