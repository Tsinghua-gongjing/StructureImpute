...
####################################################################
class AllFusionNetMultiply_SRU(nn.Module):
    def __init__(self, args):
        super(AllFusionNetMultiply_SRU, self).__init__()
...
#             self.lstm = nn.LSTM(self.channels2, self.hidden_size, self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=self.dropout) # as residual block
            self.lstm = SRU(self.channels2, self.hidden_size, self.num_layers, bidirectional=self.bidirectional, dropout=self.dropout) # as residual block
            (self.lstm).cuda()
        else:
#             self.lstm = nn.LSTM(self.input_size, self.hidden_size, self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=self.dropout)
            self.lstm = SRU(self.input_size, self.hidden_size, self.num_layers, bidirectional=self.bidirectional, dropout=self.dropout)
            (self.lstm).cuda()
...
#             self.icSHAPE = nn.LSTM(self.channels2, self.hidden_size, self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=self.dropout)
            self.icSHAPE = SRU(self.channels2, self.hidden_size, self.num_layers, bidirectional=self.bidirectional, dropout=self.dropout)
            (self.lstm).cuda()
        else:
#             self.icSHAPE = nn.LSTM(1, self.hidden_size, self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=self.dropout)
            self.icSHAPE = SRU(1, self.hidden_size, self.num_layers, bidirectional=self.bidirectional, dropout=self.dropout)
            (self.lstm).cuda()
    ...
    def forward(self, x, y):
        ...
        out1, _ = self.lstm(x, h0)  # out: tensor of shape (batch_size, seq_length, hidden_size)
        # print("y shape",y.shape)
        out2, _ = self.icSHAPE(y, h1)
